...
pub struct AgentEmploymentRepositoryImpl {
    db: Arc<DatabaseConnection>,
}

impl AgentEmploymentRepositoryImpl {
    pub fn new(db: Arc<DatabaseConnection>) -> Self {
        Self { db }
    }
}
...
#[async_trait]
impl AgentEmploymentRepository for AgentEmploymentRepositoryImpl {
    async fn employ(&self, agent_id: &AgentId, user_id: &UserId) -> Result<()> {
        // Check if employment already exists
        let existing = entities::agent_employment::Entity::find()
            .filter(entities::agent_employment::Column::AgentId.eq(agent_id.0))
            .filter(entities::agent_employment::Column::UserId.eq(user_id.0))
            .one(self.db.as_ref())
            .await?;

        if existing.is_some() {
            return Err(PlatformError::AgentAlreadyEmployed(
                format!("User {} has already employed agent {}", user_id.0, agent_id.0)
            ));
        }

        let employment = entities::agent_employment::ActiveModel {
            agent_id: Set(agent_id.0),
            user_id: Set(user_id.0),
            employed_at: Set(Utc::now()),
        };

        entities::agent_employment::Entity::insert(employment)
            .exec(self.db.as_ref())
            .await?;
    ...
    async fn terminate(&self, agent_id: &AgentId, user_id: &UserId) -> Result<()> {
        let result = entities::agent_employment::Entity::delete_many()
            .filter(entities::agent_employment::Column::AgentId.eq(agent_id.0))
            .filter(entities::agent_employment::Column::UserId.eq(user_id.0))
            .exec(self.db.as_ref())
            .await?;
        ...
        if result.rows_affected == 0 {
            return Err(PlatformError::AgentNotEmployed(
                format!("User {} has not employed agent {}", user_id.0, agent_id.0)
            ));
        }
    ...
    async fn is_employed(&self, agent_id: &AgentId, user_id: &UserId) -> Result<bool> {
        let count = entities::agent_employment::Entity::find()
            .filter(entities::agent_employment::Column::AgentId.eq(agent_id.0))
            .filter(entities::agent_employment::Column::UserId.eq(user_id.0))
            .count(self.db.as_ref())
            .await?;
    ...
    async fn find_by_agent(&self, agent_id: &AgentId) -> Result<Vec<UserId>> {
        let employments = entities::agent_employment::Entity::find()
            .filter(entities::agent_employment::Column::AgentId.eq(agent_id.0))
            .all(self.db.as_ref())
            .await?;

        Ok(employments.into_iter().map(|e| UserId::from_uuid(e.user_id)).collect())
    }
